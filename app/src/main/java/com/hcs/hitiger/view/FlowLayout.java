    ...
    protected void onLayout(boolean changed, int leftPosition, int topPosition, int rightPosition, int bottomPosition) {
        ...
        for (View child : getLayoutChildren()) {
            ...
            if ((x + childWidth) > widthOffset) {
                ...
                rowIndex = rowIndex + 1;
                int moveForAligningCenter = (widthOffset - x) / 2;
                for (View view : childViewList) {
                    int left = view.getLeft() + moveForAligningCenter;
                    view.layout(left, view.getTop(), view.getRight() + moveForAligningCenter, view.getBottom());
                }
                childViewList.clear();
            }
